...
def chunk_conceptual_delta_rule_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    w, u, A = chunk_conceptual_delta_rule_fwd_intra(
        k=k,
        v=v,
        g=g,
        beta=beta,
    ...
    if cp_context is not None:
        initial_state = chunk_conceptual_delta_rule_fwd_h_pre_process(
            k=k,
            w=w,
            u=u,
            g=g,
    ...
    h, v_new, final_state = chunk_conceptual_delta_rule_fwd_h(
        k=k,
        w=w,
        u=u,
        g=g,
...
def chunk_conceptual_delta_rule_bwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    h, v_new, _ = chunk_conceptual_delta_rule_fwd_h(
        k=k,
        w=w,
        u=u,
        g=g,
    ...
    if cp_context is not None:
        ...
        dht, initial_state = chunk_conceptual_delta_rule_bwd_dhu_pre_process(
            q=q,
            k=k,
            w=w,
            do=do,
    ...
    dh, dh0, dv = chunk_conceptual_delta_rule_bwd_dhu(
        q=q,
        k=k,
        w=w,
        g=g,
    ...
    if use_gate_in_kernel:
        dg, dA_log, ddt_bias = cdn_gate_bwd(g=g_input, A_log=A_log, dt_bias=dt_bias, dyg=dg)
...
class ChunkConceptualDeltaRuleFunction(torch.autograd.Function):
    ...
    def backward(
        ...
        dq, dk, dv, db, dg, dh0, dA_log, ddt_bias = chunk_conceptual_delta_rule_bwd(
            q=q,
            k=k,
            v=v,
            g=g,
...
def chunk_conceptual_delta_rule(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    **kwargs,
    ...
    Args:
        ...
        use_gate_in_kernel (bool):
            Whether to compute the log-space cdn decay internally.
    ...
    Examples::
        ...
        >>> from fla.ops.conceptual_delta_rule import chunk_conceptual_delta_rule
        ...
        >>> o, ht = chunk_conceptual_delta_rule(
            q, k, v, g, beta,
            initial_state=h0,
    ...
    if chunk_size not in (16, 32, 64):
        raise ValueError(f"`chunk_size` must be 16, 32, or 64 for conceptual Delta Rule, got {chunk_size}.")
...
chunk_cdn = chunk_conceptual_delta_rule
