...
def naive_cdn_gate(
    ...
    """
    Torch reference implementation for cdn gate computation.
...
def cdn_gate_chunk_cumsum_scalar_kernel(
    g,
    A_log,
    dt_bias,
    o,
...
def cdn_gate_chunk_cumsum(
    ...
    cdn_gate_chunk_cumsum_scalar_kernel[(NT, B * H)](
        g=g,
        A_log=A_log,
        dt_bias=dt_bias,
        o=o,
...
class cdnGateFunction(torch.autograd.Function):
    @staticmethod
    @input_guard
    @autocast_custom_fwd
    def forward(
        ...
        yg = cdn_gate_fwd(g=g, A_log=A_log, dt_bias=dt_bias, output_dtype=output_dtype)
    ...
    def backward(ctx, dyg: torch.Tensor):
        ...
        dg, dA, dbias = cdn_gate_bwd(g=g, A_log=A_log, dt_bias=dt_bias, dyg=dyg)
...
def fused_cdn_gate(
    ...
    r"""
    Fused cdn gate computation with autograd support.
    ...
    Returns:
        Output tensor of shape `[..., HV]`.
    """
    return cdnGateFunction.apply(g, A_log, dt_bias, output_dtype)
