...
@triton.jit
def naive_flex_attn_decoding_kernel(
    q,
    k,
    v,
    o,
    ...
    for i_s in range(0, T, BS):
        ...
        if USE_WEIGHT_BIAS:
            b_s += b_weight_bias
        b_p = tl.where(m_k, tl.sigmoid(b_s), 0.0)
...
def flex_attn_decoding_one_step(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    *,
    ...
    Args:
        ...
        sink_bias (Optional[torch.Tensor]):
            ...
            Adds `sigmoid(sink_bias + weight_bias)` to the denominator without
    ...
    grid = (NV * N * HQ,)
    naive_flex_attn_decoding_kernel[grid](
        q=q,
        k=k,
        v=v,
        o=o,
