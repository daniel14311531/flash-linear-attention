...
class MesaNetDIYBlock(GradientCheckpointingLayer):

    def __init__(self, config: MesaNetDIYConfig, layer_idx: int):
        super().__init__()
...
class MesaNetDIYPreTrainedModel(PreTrainedModel):

    config_class = MesaNetDIYConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['MesaNetDIYBlock']
    ...
    def _init_weights(
        ...
        if isinstance(module, MesaNetDIY) and next(module.parameters()).device.type != 'meta':
            with torch.no_grad():
...
class MesaNetDIYModel(MesaNetDIYPreTrainedModel):

    def __init__(self, config: MesaNetDIYConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([MesaNetDIYBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`MesaNetDIYModel` does not `output_attentions` now, setting it to `False`.")
...
class MesaNetDIYForCausalLM(MesaNetDIYPreTrainedModel, FLAGenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MesaNetDIYModel(config)
        self.vocab_size = config.vocab_size
