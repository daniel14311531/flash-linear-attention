...
class MesaNetDIY(nn.Module):
    """
    The layer implementaion modified from [MesaNet: Sequence Modeling by Locally Optimal Test-Time Training].  # noqa
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Pre-allocated lambda tensor (all 1s) to avoid per-forward allocation issues in multi-GPU training
        # self.register_buffer(
        #     'lamb',
        #     torch.ones(self.num_heads, self.head_dim, dtype=torch.float32),
        # )

        # Gated gate parameters (from GatedDeltaNet / Mamba2 style discretization)
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # Mamba2-style gated discretization: g = dt * A where dt > 0, A < 0
        dt = F.softplus(self.a_proj(hidden_states).float() + self.dt_bias)
