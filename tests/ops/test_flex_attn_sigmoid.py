...
from fla.ops.flex_attn.sigmoid.decoding import flex_attn_decoding_one_step
from fla.ops.flex_attn.sigmoid.naive import naive_flex_attn_decoding, naive_parallel_flex_attn
from fla.ops.flex_attn.sigmoid.parallel import parallel_flex_attn
...
def test_parallel_varlen():
    ...
    for bos, eos in zip(cu_seqlens[:-1].tolist(), cu_seqlens[1:].tolist(), strict=False):
        outputs.append(naive_parallel_flex_attn(
            q=q_ref[:, bos:eos],
            k=k_ref[:, bos:eos],
            v=v_ref[:, bos:eos],
            g=g_ref[:, bos:eos],
